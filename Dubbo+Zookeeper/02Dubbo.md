...
随着Internet的快速发展，Web应用程序的规模不断扩大，最终我们发现传统的垂直架构（单片机）无法再解决这个问题。分布式服务架构和流量计算架构势在必行，迫切需要一个治理系统来确保架构的有序演进。

![图片](/Users/liujiang/Documents/Typora/imgs/dubbo-architecture-roadmap.jpg)
...
![图片](/Users/liujiang/Documents/Typora/imgs/dubbo-service-governance.jpg)

在大型服务出现之前，应用程序可能只使用RMI或Hessian公开或引用远程服务，通过配置严格的URL来完成调用，并通过硬件（如F5）完成负载平衡。

**当服务越来越多时，配置服务URL变得非常困难，F5硬件负载均衡器的单点压力也在增加。**此时，需要服务注册表来动态注册和发现服务，以使服务的位置透明。通过获取消费者侧的服务提供商地址列表，可以实现软负载平衡和故障转移，这减少了对F5硬件负载的依赖性和一些成本。

**当事情进一步发展时，服务依赖性变得如此复杂，以至于甚至无法确定之前启动哪些应用程序，即使架构师也无法完全描述应用程序架构关系**。这时，需要自动绘制应用程序的依赖关系图，以帮助架构师明确关系。

**然后，流量变得更重，服务的容量问题暴露出来，需要多少台机器才能支持这项服务？什么时候应该添加机器？**要解决这些问题，首先应将每日服务呼叫和响应时间量作为容量规划的参考。其次，动态调整重量，增加在线机器的重量，并记录响应时间变化，直到达到阈值，记录此时的访问时间，然后将这个访问次数乘以机器总数来计算容量反过来。

以上是达博最基本的要求。

# 建筑

![达博-architucture](/Users/liujiang/Documents/Typora/imgs/dubbo-architecture.jpg)
...
2. `Provider``Register`在其启动时注册其服务。
3. `Consumer`从`Register`它开始时订阅它所需的服务。
4. `Register`返回`Provider`s列表`Consumer`，当它更改时，`Register`将`Consumer`通过长连接推送更改的数据。
5. `Consumer`选择一个`Provider`基于软负载均衡算法的s并执行调用，如果失败，则选择另一个`Provider`。
6. 双方`Consumer`并`Provider`会数数的服务调用和耗时的内存，并发送统计`Monitor`每一分钟。

Dubbo具有以下功能：连接性，稳健性，可扩展性和可升级性。

## 连接

- `Register`负责注册和搜索服务地址，如目录服务，`Provider`并且`Consumer`只在启动期间与注册表交互，并且注册表不转发请求，因此压力较小
- 'Monitor'负责计算服务调用次数和耗时，统计信息将首先在`Provider`'s和`Consumer`'s内存中汇编，然后发送到`Monitor`
- 'Provider'将服务注册到'Register'并向'Monitor'报告耗时的统计信息（不包括网络开销）
- 'Consumer'获取服务提供者地址列表，`Registry`根据LB算法直接调用提供者，报告耗时的统计信息`Monitor`，其中包括网络开销
- 之间的连接`Register`，`Provider`并且`Consumer`是长连接，`Moniter`是一个例外
- `Register`意识到`Provider`通过长连接的存在，当`Provider`下来时，`Provider`将推动事件`Consumer`
- 它不影响已经运行的实例`Provider`和`Consumer`甚至全部的`Register`和`Monitor`趴下，因为`Consumer`得到的缓存`Provider`上榜
- `Register`并且`Monitor`是可选的，`Consumer`可以`Provider`直接连接

## 稳健性

- `Monitor`停机时间不会影响使用情况，只会丢失一些采样数据
- 当数据库服务器出现故障时，`Register`可以通过检查其缓存来返回服务`Provider`列表`Consumer`，但是新服务器`Provider`无法注册任何服务
- `Register` 是一个对等集群，它会在任何实例出现故障时自动切换到另一个集群
- 甚至所有`Register`的实例都会崩溃，`Provider`并且`Consumer`仍然可以通过检查其本地缓存来进行通信
- 服务`Provider`是无状态的，一个实例的停机时间不会影响使用
- 在`Provider`一个服务的所有s下来后，`Consumer`不能使用该服务，并无限重新连接等服务`Provider`恢复

## 可扩展性

- `Register` 是一个可以动态增加其实例的对等集群，所有客户端都将自动发现新实例。
- `Provider`是无状态的，它可以动态增加部署实例，并且注册表将新的服务提供者信息推送到`Consumer`。

## Upgradeablity

当服务集群进一步扩展并且IT治理结构进一步升级时，需要动态部署，并且当前的分布式服务体系结构不会带来阻力。这是一个可能的未来架构：

![达博-architucture期货](/Users/liujiang/Documents/Typora/imgs/dubbo-architecture-future.jpg)
...
## 安装管理界面(前提先启动zookeeper)

- 下载dubbo-admin(incubator-dubbo-ops-master.zip)

- 进入admin

- 修改配置文件(admin/src/resources/application.properties)
...
- 配置服务提供者,在resources文件夹下创建provider.xml配置文件
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!-- 声明需要暴露的服务接口 ref指向服务真正实现对象-->
    <dubbo:service interface="com.atguigu.gmall.service.UserService" ref="userServiceImpl" />
 
    <!-- 和本地bean一样实现服务 -->
    <bean id="userServiceImpl" class="com.aishang.service.UserServiceImpl" />
</beans>
```
...
- pom
  ...
  <project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://maven.apache.org/POM/4.0.0 http://maven.apache.org/xsd/maven-4.0.0.xsd">
    ...
    <groupId>com.aishang.gmall</groupId>
...
<beans xmlns="http://www.springframework.org/schema/beans"
	...
	<context:component-scan
		base-packeage="com.aishang.service.OrderServiceImpl"></context:component-scan>
...
public class Consumer {
	public static void main(String[] args) throws IOException {
		...
		for (UserAddress userAddress : initOrder) {
...
实现类OrderServiceImpl.java,注意这里的注解是spring的@service，先不要使用dubbo的
...
# 使用springboot方式搭建dubbo

- 创建boot-user-provider

![Window_和_eclipse-workspace_-_Eclipse](https://ws1.sinaimg.cn/large/006tKfTcly1g13hjb4t3vj30f80ky75t.jpg)

![New_Module_和_dubbo-shop-final____Documents_IDEA_dubbo-shop-final__和_dubbo-shop____Documents_IDEA_dubbo-shop__-_____dubbo-user-service-consumer_src_main_java_com_aishang_shop_ConsumerApplication_java__dubbo-user-service-consumer_](https://ws1.sinaimg.cn/large/006tNc79ly1g2ruwuil97j31750u00uu.jpg)
...
<project xmlns="http://maven.apache.org/POM/4.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
	...
	<artifactId>boot-user-service-prodiver</artifactId>
	<version>0.0.1-SNAPSHOT</version>
	<name>boot-user-service-prodiver</name>
...
- 创建消费者
  ...
  ![截屏2019_3_15_15_06](https://ws1.sinaimg.cn/large/006tKfTcly1g13hu34wbpj30ed0epgmq.jpg)

![截屏2019_3_15_15_07](https://ws3.sinaimg.cn/large/006tKfTcly1g13hut9nukj30ek0jcwfq.jpg)
...
- provider配置  application.properties

  ```xml
  
  dubbo.application.name=dubbo-user-service-provider
  dubbo.registry.address=zookeeper://172.31.12.236:2181?backup=172.31.12.236:2182,172.31.12.236:2183
...
- 暴露服务的注解
  ...
  import com.atguigu.gmall.bean.UserAddress;
  import com.atguigu.gmall.service.UserService;
  @Service//阿里的，但是还需要spring的service重名了，用下面这个代替
  @Component
  public class UserServiceImpl implements UserService{
  
  	public List<UserAddress> getUserAddressList(String userId) {
  		// TODO Auto-generated method stub
  		System.out.println("userservice id="+userId);
  		UserAddress userAddress1  = new UserAddress(1,"哈尔滨",userId,"tom","18100001111","默认地址");
  		UserAddress userAddress2  = new UserAddress(2,"哈尔滨",userId,"jerry","18100001111","默认地址");
...
- 主程序中要开启dubbo
  ...
  public class BootUserServiceProdiverApplication {
  
  	public static void main(String[] args) {
  		SpringApplication.run(BootUserServiceProdiverApplication.class, args);
  	}
...
![img](http://ww2.sinaimg.cn/large/006tNc79ly1g58qqv0nqej318o0kmgnx.jpg)

下面来看看一致性 hash 在 Dubbo 中的应用。我们把上图的缓存节点替换成 Dubbo 的服务提供者，于是得到了下图：

![img](http://ww4.sinaimg.cn/large/006tNc79ly1g58qqvjpx1j31920kiq51.jpg)

这里相同颜色的节点均属于同一个服务提供者，比如 Invoker1-1，Invoker1-2，……, Invoker1-160。这样做的目的是通过引入虚拟节点，让 Invoker 在圆环上分散开来，避免数据倾斜问题。所谓数据倾斜是指，由于节点不够分散，导致大量请求落到了同一个节点上，而其他节点只会接收到了少量请求的情况。比如：

![img](http://ww4.sinaimg.cn/large/006tNc79ly1g58qqzbnxbj318a0j4jtv.jpg)
