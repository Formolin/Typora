...
OpenVAS 是开源的网络漏洞扫描器。自从Nessus 收费以后，分支出来的项目
...
sudo gvm-setup
看到：It seems like your GVM-11 installation is OK，说明安装完成了，没有问题。
sudo gvm-feed-update //第一次安装后，不用升级,但后期使用记得经常升级特征库
#修改密码
sudo runuser -u _gvm -- gvmd --user=admin --new-password=admin
//gvmd前是两个“-”加空格。
如果未修改成功，在运行一遍gvm-check-setup
创建账号的命令：
runuser -u _gvm – gvmd --create-user=root（非root用户的前面加个sudo即可）会生成一个随机密码，使用上面修改admin的方法修改新建的账号。
（注：gvmd前面是2个‘-’以及一个空格，create前面是2个‘-’，编辑时是2个‘-’，文章发出来后只剩一个‘-’，这里建议手打）
注：使用runuser -u _gvm – gvmd --create-user=root --new-password=1234依旧会生成随机密码。


#开启服务,在kali中搜索gvm命令，网路桥接自动检测
gvm-start，等待5秒，打开浏览器
报链接不安全（火狐）
先点击advance,再点击add exception
#浏览器登录
地址：http://ip:9392


之前就已经apt-get install openvas 了，而且发现删不干净。。
这个其实也无所谓
1).先做个清理
apt-get autoclean
...
目前只能在虚拟中打开浏览器访问，但是，一般在虚拟机里安装的漏扫系统在虚拟机里用起来不太顺手。可以修改一些文件使得可以在本机进行访问。

```sh
#首先修改green-security-assistant.service:
cat /lib/systemd/system/green-security-assistant.service
[Service]
Type=forking
User=_gvm
Group=_gvm
...
#这个将listen改成0.0.0.0任意ip，增加host 头主机地址（IP或域名）
#在--mlisten=0.0.0.0 后增加“--allow-header-host=外部访问的地址IP或域名”，本次测试本机地址为：192.168.200.221，即外部访问的IP为192.168.200.221

ExecStart=/usr/sbin/gsad --foreground --listen=0.0.0.0 --port=9392 --mlisten=0.0.0.0 --allow-header-host=192.192.37.160 --mport=9392
#重新加载
...
cat /lib/systemd/system/openvas-manager.service
cat /etc/default/openvas-manager

修改/etc/defauilt/greenbone-security-assistant.修改两处

```


![img](50、虚拟机安装OpenVAS/857062-20180718122527476-416572044.png) 

![img](50、虚拟机安装OpenVAS/857062-20180718122557567-417206042.png)
...
![image-20201201190422211](50、虚拟机安装OpenVAS/image-20201201190422211.png)
...
按 esc
 ...
 chmod 777 installd.sh
 ./installd.sh
 
 
 第二步:安装docker-compose(快速启动器)
